...
- 检索器配置了 k 值（每次返回的文档数量），通过 config_data.top_k 控制
...
class VectorStoreService(object):
    def __init__(self,embedding):
        ...
        self.vector_store = Chroma(
            ...
            collection_configuration=config.chroma_collection_configuration,
        )

    def get_retriever(self):
        """返回向量检索器,方便加入chain

        使用 similarity_score_threshold 检索类型：
        只返回余弦相似度 >= config.similarity_score_threshold 的片段，
        避免把与问题无关的内容硬塞给 LLM 导致幻觉。
        """
